"""Register IXI MRA and vessel-segmentation volumes into padded MNI space.

Inputs follow the repository's ``data/`` layout.  Registered MRA volumes and
binary segmentations are written to ``results/registration/ixi``.  Run with
``.\\.venv\\Scripts\\python.exe processing\\registration\\ANT_register_mra_t1_mni_batch_args.py``;
all paths can be overridden with the documented command-line flags.
"""
...
parser = argparse.ArgumentParser(
    description="Batch rigid registration of IXI MRA data to MNI space via T1 brain images."
)
parser.add_argument(
    "--mni-path",
...
parser.add_argument(
    "--t1-dir",
    default=os.path.join(base_path, 'data', 'T1-BrainOnly', 'IXI-T1-BrainOnly'),
    help="Directory containing brain-only T1 images."
)
parser.add_argument(
    "--mra-dir",
    default=os.path.join(base_path, 'data', 'MRA', 'IXI-MRA'),
    help="Directory containing the original IXI MRA images."
)
parser.add_argument(
    "--seg-dir",
    default=os.path.join(base_path, 'data', 'Segmentation', 'IXI-Manual'),
    help="Directory containing the IXI manual segmentations."
)
parser.add_argument(
    "--out-dir",
    default=os.path.join(base_path, 'results', 'registration', 'ixi'),
...
# ---------------------------------------------------------
# Find all T1 brain files instead of MRA brains
t1_brain_files = glob.glob(os.path.join(t1_dir, "my_brain_*.nii*"))
print(f"\n🚀 Found {len(t1_brain_files)} T1 Brain files. Starting batch registration...\n")

for t1_path in t1_brain_files:
    filename = os.path.basename(t1_path)

    # Extract ID (e.g., '145' from 'my_brain_145.nii.gz')
    match = re.search(r'my_brain_(\d+)', filename)
    if not match:
        continue

    scan_id = match.group(1)

    # Locate corresponding full files
    ...
    if not mra_match or not seg_match:
        print(f"⚠️ Missing Original MRA or Seg match for ID {scan_id}. Skipping.")
        continue
    ...
    out_mra = os.path.join(out_dir, f"ANTs_MNI_Full_MRA_{scan_id}.nii.gz")
    out_seg = os.path.join(out_dir, f"ANTs_MNI_Seg_{scan_id}.nii.gz")
    temp_seg_path = os.path.join(out_dir, f"temp_fixed_seg_{scan_id}.nii.gz")

    # Skip if already fully processed
    if os.path.exists(out_mra) and os.path.exists(out_seg):
        print(f"⏭️ ID {scan_id} already exists. Skipping.")
        continue
    ...
    try:
        # ---------------------------------------------------------
        # A. Repair Segmentation Metadata (SimpleITK)
        # ---------------------------------------------------------
        ...
        print(f"✅ Finished ID {scan_id}\n")

    except Exception as e:
        print(f"❌ Failed ID {scan_id}: {e}\n")
...
print("🎯 All scans have been successfully aligned to MNI space using ANTs!")
