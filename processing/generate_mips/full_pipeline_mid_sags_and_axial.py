"""Generate standardized TubeTK MIPs and cross-view metadata.

This self-contained pipeline registers each TubeTK case, creates sagittal,
posterior axial, and anterior coronal MIPs, and stores their geometry metadata.
Usage: ``python processing/generate_mips/full_pipeline_mid_sags_and_axial.py
--input_dir data --output_dir results/mips/tubetk --model_name <name>``.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "TubeTK MRA registration, sagittal crop, "
            "posterior and anterior extraction pipeline."
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Base directory containing MNI, T1, MRA "
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Segmentation model name. Used in the "
    ...
    t1_dir = os.path.join(args.input_dir, "T1-BrainOnly", "TubeTK-T1-BrainOnly")
    mra_dir = os.path.join(args.input_dir, "MRA", "TubeTK-MRA")
    seg_dir = os.path.join(args.input_dir, "Segmentation", "TubeTK-Manual")
    ...
    print("\n" + "=" * 70)
    print(f"INITIALIZING TUBETK PIPELINE FOR MODEL: {args.model_name}")
    print("=" * 70)
    ...
    print(f"🚀 Found {len(t1_brain_files)} scans.\n")
    ...
    for t1_path in t1_brain_files:
        ...
        match = re.search(r"output_Brain_Normal-(\d+)_MRA", os.path.basename(t1_path))
        if not match:
            continue
        ...
        if not mra_match or not seg_match:
            print(f"⚠️ Missing Original MRA or Segmentation for Normal-{scan_id}. Skipping.")
            continue

        mra_path = mra_match[0]
        seg_path = seg_match[0]

        standardized_name = f"TubeTK_{args.model_name}_{scan_id}.png"

        print("\n" + "=" * 70)
        print(f"PROCESSING: Normal-{scan_id}")
        ...
        try:
            ...
            # STEP 1: REGISTER MRA
            # =================================================
            t1_to_mni_mat = os.path.join(trans_dir, f"T1_to_MNI_Normal-{scan_id}.mat")
            mra_to_t1_mat = os.path.join(trans_dir, f"MRA_to_T1_Normal-{scan_id}.mat")
            reg_mra_path = os.path.join(mra_out_dir, f"ANTs_MNI_MRA_Normal-{scan_id}.nii.gz")
            ...
            # =================================================
            reg_seg_path = os.path.join(seg_out_dir, f"ANTs_MNI_Seg_Normal-{scan_id}.nii.gz")
            ...
            # Sagittal background MIP (Flipped horizontally by omitting previous fliplr logic)
            ...
            for config_name, offset_left, offset_right in slice_configs:
                ...
                # Sagittal vessel MIP (Flipped horizontally by omitting previous fliplr logic)
            ...
            print(f"\n    ✅ COMPLETE: Normal-{scan_id}")
            print(f"       Filename: {standardized_name}")

        except Exception as e:
            print(f"\n    ❌ FAILED: Normal-{scan_id}")
            print(f"       Error: {e}")
